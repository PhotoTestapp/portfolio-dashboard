...
不足入力テンプレートCSVの取込は、即時反映ではなくプレビュー方式です。

1. `不足入力CSV取込` でCSVを選択
2. 反映予定件数、HIGH影響件数、不正行、スキップ行を確認
3. 変更前→変更後、判定変化見込み、影響度を確認
4. 問題なければ `プレビューを適用`
5. 取り消す場合は `キャンセル`
...
カバレッジ診断、停止判定、リスク優先度から、次に入力すべき項目を自動生成します。

### 追加出力

- `portfolio-bulk-input-suggestions.csv`
  - 作業指示用CSV
  - 銘柄、入力項目、理由、必要根拠、入力ヒント、優先度を出力
- `portfolio-bulk-paste-template.tsv`
  - 一括貼り付け入力欄にそのまま貼るためのTSVテンプレート
  - `code / missingField / importedValue` 形式

### 運用手順

1. 入力候補CSVを出力
2. 必要根拠を確認
3. 貼付用TSVまたは表計算ソフトで値を入力
4. 一括貼り付け入力へ貼り付け
5. プレビュー確認
6. 適用
7. auditLogと判定結果を確認

この機能は入力すべき項目を提示するだけです。値そのものの根拠確認は引き続き必要です。

## 安全操作モード

通常状態は `閲覧モード` です。閲覧モードでは入力欄、取込、復元、削除、ルール変更などの変更操作をロックします。

編集する場合は、画面上部の `編集モード解除` を押し、確認文字列 `EDIT` を入力します。作業後は `閲覧モードへ戻す` を押してください。
...
切替操作は auditLog に記録されます。静的GitHub Pagesのため厳密なユーザー認証はありません。

## ガイドワークフロー

日次・週次・月次・四半期の運用作業を順番固定する `guidedWorkflow` を追加。
...
- 各ワークフローのステップ進捗管理
- 運用チェックリストとの連動
- ステップ完了時の `auditLog` 記録
- ワークフローCSV出力 `portfolio-guided-workflows.csv`
- 閲覧モードでは進捗更新不可。編集モードでのみ操作可能

### 目的

人間が作業順を選ばない構造にし、確認漏れ・順序違い・月次レビュー漏れを減らす。

## 未解決リスク優先改修キュー

静的 GitHub Pages 構成で残る未解決項目を、現在のデータ状態から機械的に優先順位化します。

### 追加項目

- 株価・為替・財務データ自動取得なし
- 根拠URL本文との自動照合なし
- localStorage依存・複数端末同期なし
- 認証・権限管理なし
- 証券口座・約定データ連携なし
- 結果評価データ不足
...
`portfolio-unresolved-risk-priority.csv`

### 注意

静的GitHub Pages単体で完全解決できない項目は、代替策と次アクションを表示します。

## データ更新パック生成

静的GitHub Pagesでは株価・為替・財務データの自動取得はできないため、未解決リスクを作業単位に分解したCSVを出力できます。

追加された出力:

| ファイル | 用途 |
|---|---|
| `portfolio-daily-update-pack.csv` | 現在価格・価格更新日・USD/JPY更新日の確認 |
| `portfolio-weekly-evidence-pack.csv` | 根拠URL、引用、採用証跡値、実行/結果未入力の確認 |
| `portfolio-monthly-operation-pack.csv` | 判定履歴保存、運用レポート、ルール確認、JSONバックアップ |
| `portfolio-quarterly-financial-pack.csv` | 配当性向、営業CF、EPS、自己資本比率、業種別専用指標の更新 |
| `portfolio-all-data-update-pack.csv` | 上記すべてを統合した作業CSV |

各CSVは作業指示用です。値を埋めた後は、一括貼り付け入力または不足入力CSV取込で反映します。

## 外部基盤連携・自動化移行キュー

未解決だった以下の項目を、静的GitHub Pagesでできる代替運用と、外部基盤へ移行するための仕様に分解しました。
...
- `portfolio-external-integration-plan.csv`
- `portfolio-backend-migration-spec-YYYY-MM-DD.json`
- `portfolio-external-data-connector-template.csv`
- `portfolio-evidence-url-verification-pack.csv`

### 重要な制約

GitHub Pages単体では、APIキー秘匿、サーバー保存、認証、証券口座連携、URL本文取得・PDF解析を安全に完結できません。そのため、この改修では「現構成での代替運用」と「外部基盤に移行するためのAPI仕様・作業CSV」を追加しています。

### 運用手順

1. 外部基盤連携・自動化移行キューで優先順位を確認する。
2. `API仕様JSON` を出力し、バックエンド/API設計のたたき台にする。
3. `外部データ取込テンプレート` を出力し、外部データ取得結果の列形式を固定する。
4. `URL本文照合Pack` を出力し、根拠URL・引用文・採用値の検証対象を固定する。
5. 外部基盤ができるまでは、既存のデータ更新パック・一括貼り付け・CSV取込を継続する。

## Supabase最小バックエンド移行キット

この版では、GitHub Pages単体では解消できない `localStorage依存・認証なし・複数端末同期なし` を次段階で潰すための最小Supabase移行ファイルを追加しています。
...
src/config/backendMigrationSpec.js
```
...
| 課題 | Supabase移行後の対応 |
|---|---|
| localStorage依存 | `portfolio_state` テーブルへ保存 |
| 複数端末同期 | user_id単位で同一stateを同期 |
| 認証なし | Supabase Authを利用 |
| 監査ログの端末依存 | `audit_log` テーブルへ保存 |
| 判定履歴の端末依存 | `decision_history` テーブルへ保存 |
...
旧詳細画面を壊さず、画面上部に「今日見るべきこと」を追加しました。
...
前回の大規模タブ分割で白画面が発生したため、今回は既存UIを保持したまま上部に要約ブロックのみ追加しています。


## 入力項目階層化

銘柄カードに `Level 1 最小 / Level 2 財務 / Level 3 証跡` の入力モードを追加しました。

- Level 1: 保有数、取得単価、現在価格、年間配当のみ表示
- Level 2: 財務指標、判定プロファイル、プロファイル専用指標、更新日を表示
- Level 3: 根拠URL、引用文、採用証跡値などの証跡入力を表示

日常運用ではLevel 1を基本にし、SELL / REDUCE / 停止判定 / リスク上位銘柄だけLevel 2またはLevel 3を開いて更新します。


## 入力負荷軽減: LOW_CONFIDENCE 判定

全銘柄に同じ証跡厳格チェックを要求しないため、証跡不備の扱いを分岐しました。

- SELL / REDUCE / BUY など実行判断に直結する銘柄は、従来どおり `UNVERIFIED_DATA` / `WEAK_EVIDENCE` / `MULTIPLE_EVIDENCE_VALUES` / `MISMATCHED_EVIDENCE` で通常判定を停止します。
- HOLD / WATCH など低リスク・実行対象外の銘柄は、証跡不備があっても `LOW_CONFIDENCE` として参考判定に緩和します。
- これにより、全銘柄にLevel 3証跡入力を要求せず、重要銘柄から入力する運用に寄せています。

### 実務上の使い方

1. Level 1 / Level 2 の入力で全体を監視する。
2. SELL / REDUCE / BUY / riskPriority上位のみ Level 3 証跡を入力する。
3. LOW_CONFIDENCE は低優先度として扱い、日次処理では上位リスク銘柄を優先する。
