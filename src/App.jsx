...
export default function PortfolioManagementDashboard() {
  ...
  const [activeDashboardTab, setActiveDashboardTab] = useState('command')
  const [expandedFocusCard, setExpandedFocusCard] = useState('')
  ...
  const focusBlockingDecisions = ['INVALID_DATA', 'UNVERIFIED_DATA', 'WEAK_EVIDENCE', 'MULTIPLE_EVIDENCE_VALUES', 'MISMATCHED_EVIDENCE', 'PROFILE_DATA_REQUIRED', 'RULE_CONFIG_REQUIRED', 'STALE_DATA', 'NO_DATA']
  const focusEvidenceDecisions = ['UNVERIFIED_DATA', 'WEAK_EVIDENCE', 'MULTIPLE_EVIDENCE_VALUES', 'MISMATCHED_EVIDENCE']
  const focusActionDecisions = ['SELL', 'REDUCE']
  const focusNeedsAction = riskPriorityList
    .filter((stock) => focusActionDecisions.includes(stock.decisionResult?.decision) || focusBlockingDecisions.includes(stock.decisionResult?.decision) || ['CRITICAL', 'HIGH'].includes(stock.riskPriorityLevel))
    .slice(0, 10)
  const focusInputNeeds = bulkInputSuggestions.slice(0, 10)
  const focusEvidenceNeeds = riskPriorityList.filter((stock) => focusEvidenceDecisions.includes(stock.decisionResult?.decision)).slice(0, 10)
  const focusExecutionNeeds = decisionHistory
    .filter((item) => ['SELL', 'REDUCE', 'BUY'].includes(item.decision) && ['NOT_EXECUTED', 'CONTRADICTED', 'NON_COMPLIANT', '', undefined].includes(item.complianceStatus))
    .slice(0, 10)
  const focusAuditItems = auditLog.slice(0, 10)
  const focusTaskRows = guidedWorkflowStats.map((workflow) => ({ ...workflow, sortScore: (workflow.doneCount < workflow.totalSteps ? 1 : 0) * 100 + (workflow.cadence === 'daily' ? 40 : workflow.cadence === 'weekly' ? 30 : workflow.cadence === 'monthly' ? 20 : 10) })).sort((a, b) => b.sortScore - a.sortScore).slice(0, 4)

  const focusTabs = [
    { id: 'command', label: '司令塔', hint: '最初に見る' },
    { id: 'risks', label: 'リスク銘柄', hint: '日次' },
    { id: 'input', label: '入力更新', hint: '不足値' },
    { id: 'evidence', label: '証跡確認', hint: '根拠' },
    { id: 'execution', label: '実行・結果', hint: '売買後' },
    { id: 'history', label: '履歴分析', hint: '月次' },
    { id: 'audit', label: '監査ログ', hint: '異常時' },
    { id: 'settings', label: '設定・同期', hint: '必要時' },
    { id: 'legacy', label: '全詳細', hint: '旧画面' },
  ]

  const compactDecisionTone = (decision) => {
    if (decision === 'SELL') return 'bg-red-100 text-red-700 border-red-200'
    if (decision === 'REDUCE') return 'bg-orange-100 text-orange-700 border-orange-200'
    if (['INVALID_DATA', 'UNVERIFIED_DATA', 'WEAK_EVIDENCE', 'MULTIPLE_EVIDENCE_VALUES', 'MISMATCHED_EVIDENCE'].includes(decision)) return 'bg-purple-100 text-purple-700 border-purple-200'
    if (decision === 'STALE_DATA') return 'bg-amber-100 text-amber-700 border-amber-200'
    if (decision === 'BUY') return 'bg-blue-100 text-blue-700 border-blue-200'
    if (decision === 'HOLD') return 'bg-emerald-100 text-emerald-700 border-emerald-200'
    return 'bg-slate-100 text-slate-600 border-slate-200'
  }

  const FocusHeader = ({ title, description, children }) => (
  ...
  const CompactStockRow = ({ stock, rank }) => {
    const isExpanded = expandedFocusCard === stock.code
    const decision = stock.decisionResult?.decision || 'NO_DATA'
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              {rank && <span className="rounded-full bg-slate-900 px-2 py-1 text-[11px] font-black text-white">#{rank}</span>}
              <span className="font-black text-slate-900">{stock.code}</span>
              <span className="text-sm font-bold text-slate-500">{stock.name}</span>
              <span className={`rounded-full border px-2 py-1 text-[10px] font-black ${compactDecisionTone(decision)}`}>{decision}</span>
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-black text-slate-600">{stock.ruleProfile}</span>
            </div>
            <div className="mt-2 text-xs font-semibold text-slate-500">{stock.group} / {stock.market} / 更新: 価格 {stock.priceUpdatedAt || '未入力'}・財務 {stock.financialUpdatedAt || '未入力'}</div>
          </div>
          <div className="grid grid-cols-3 gap-2 text-right text-xs md:min-w-[300px]">
            <div className="rounded-xl bg-slate-50 p-2"><div className="font-black text-slate-900">{Math.round(stock.riskPriorityScore || 0)}</div><div className="text-slate-500">Risk</div></div>
            <div className="rounded-xl bg-slate-50 p-2"><div className="font-black text-slate-900">{formatPercent(stock.positionWeight)}</div><div className="text-slate-500">比率</div></div>
            <div className="rounded-xl bg-slate-50 p-2"><div className="font-black text-slate-900">{formatPercent(stock.unrealizedGainRate)}</div><div className="text-slate-500">損益率</div></div>
          </div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2 text-[11px] font-bold text-slate-600">
          {(stock.riskDrivers || []).slice(0, 3).map((driver) => <span key={driver} className="rounded-full bg-red-50 px-2 py-1 text-red-700">{driver}</span>)}
          {(stock.decisionResult?.reasons || []).slice(0, 2).map((reason) => <span key={reason} className="rounded-full bg-slate-100 px-2 py-1">{reason}</span>)}
        </div>
        <button type="button" onClick={() => setExpandedFocusCard(isExpanded ? '' : stock.code)} className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-black text-slate-700 hover:bg-white">{isExpanded ? '詳細を閉じる' : '詳細を開く'}</button>
        {isExpanded && (
          <div className="mt-3 grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"><div className="font-black text-slate-700">判定理由</div><div className="mt-1 text-slate-600">{(stock.decisionResult?.reasons || ['理由なし']).join(' / ')}</div></div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"><div className="font-black text-slate-700">不足・証跡</div><div className="mt-1 text-slate-600">{stock.sourceMetricName || '指標未指定'} / p.{stock.sourcePage || '未入力'} / {stock.sourceUnit || '単位未入力'}</div></div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"><div className="font-black text-slate-700">次アクション</div><div className="mt-1 text-slate-600">{focusActionDecisions.includes(decision) ? '実行管理でactionを記録' : focusBlockingDecisions.includes(decision) ? '入力更新または証跡確認で停止理由を解消' : '日次確認のみ'}</div></div>
          </div>
        )}
      </div>
    )
  ...
  const renderFocusedView = () => {
    if (activeDashboardTab === 'risks') {
      return (
        <section className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="リスク銘柄" description="SELL / REDUCE / 停止判定 / riskPriority上位だけを表示。通常確認はここだけで足りる構成。">
            <button type="button" onClick={exportRiskPriorityCsv} className="rounded-2xl border border-red-300 bg-red-50 px-4 py-2 text-xs font-black text-red-700">リスクCSV</button>
          </FocusHeader>
          <div className="space-y-3">{riskPriorityList.slice(0, 30).map((stock) => <CompactStockRow key={stock.code} stock={stock} rank={stock.riskPriorityRank} />)}</div>
        </section>
      )
    }
    if (activeDashboardTab === 'input') {
      return (
        <section className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="入力更新" description="不足項目・貼付用TSV・更新Packだけを集約。編集欄はここに隔離。">
            <button type="button" onClick={exportBulkPasteTemplateTsv} className="rounded-2xl border border-cyan-300 bg-cyan-50 px-4 py-2 text-xs font-black text-cyan-700">貼付用TSV</button>
            <button type="button" onClick={exportMissingDataTemplateCsv} className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-2 text-xs font-black text-amber-700">不足CSV</button>
            <button type="button" onClick={exportAllDataUpdatePackCsv} className="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-xs font-black text-slate-700">全更新Pack</button>
          </FocusHeader>
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <h3 className="font-black text-slate-900">次に埋める項目 Top10</h3>
              <div className="mt-3 space-y-2">{focusInputNeeds.map((item, index) => <div key={`${item.code}-${item.suggestedField}-${index}`} className="rounded-xl bg-white p-3 text-xs"><span className="font-black">{item.code} {item.name}</span> / {item.suggestedField} / 優先 {Math.round(item.priority || 0)}<div className="text-slate-500">{item.suggestedReason || item.requiredEvidence}</div></div>)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <h3 className="font-black text-slate-900">一括貼り付け</h3>
              <textarea value={bulkPasteText} onChange={(event) => setBulkPasteText(event.target.value)} className="mt-3 h-52 w-full rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs font-mono" placeholder={"code\tmissingField\timportedValue\n2914\tpayoutRatio\t62.4"} />
              <div className="mt-3 flex gap-2"><button type="button" onClick={previewBulkPasteImport} className="rounded-xl bg-teal-600 px-4 py-2 text-xs font-black text-white">プレビュー</button><button type="button" onClick={clearBulkPasteInput} className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-black">クリア</button></div>
            </div>
          </div>
        </section>
      )
    }
    if (activeDashboardTab === 'evidence') {
      return (
        <section className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="証跡確認" description="根拠未確認・証跡不足・不一致だけを表示。URLや引用文は必要時だけ確認。">
            <button type="button" onClick={exportEvidenceUrlVerificationPackCsv} className="rounded-2xl border border-purple-300 bg-purple-50 px-4 py-2 text-xs font-black text-purple-700">URL照合Pack</button>
          </FocusHeader>
          <div className="space-y-3">{focusEvidenceNeeds.map((stock, index) => <CompactStockRow key={stock.code} stock={stock} rank={index + 1} />)}{focusEvidenceNeeds.length === 0 && <div className="rounded-2xl bg-emerald-50 p-6 text-sm font-bold text-emerald-700">証跡系の停止判定はありません。</div>}</div>
        </section>
      )
    }
    if (activeDashboardTab === 'execution') {
      return (
        <section className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="実行・結果" description="判定、実行、結果を分離して確認。未実行SELL/REDUCEを優先。">
            <button type="button" onClick={exportDecisionHistoryCsv} className="rounded-2xl border border-blue-300 bg-blue-50 px-4 py-2 text-xs font-black text-blue-700">履歴CSV</button>
          </FocusHeader>
          <div className="grid gap-4 md:grid-cols-3"><MetricCard label="履歴件数" value={decisionHistory.length} subLabel="保存済み判定" tone="slate" /><MetricCard label="未実行候補" value={focusExecutionNeeds.length} subLabel="BUY/SELL/REDUCE" tone={focusExecutionNeeds.length ? 'amber' : 'emerald'} /><MetricCard label="評価済み" value={decisionHistory.filter((item) => item.decisionAccuracy).length} subLabel="outcomeあり" tone="sky" /></div>
          <div className="mt-4 space-y-2">{focusExecutionNeeds.map((item, index) => <div key={`${item.code}-${item.createdAt}-${index}`} className="rounded-xl border border-slate-200 bg-white p-3 text-xs"><span className="font-black">{item.code} {item.name}</span> / {item.decision} / {item.complianceStatus || '未記録'} / {item.decisionDate || item.createdAt}</div>)}</div>
        </section>
      )
    }
    if (activeDashboardTab === 'history') {
      return (
        <section className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="履歴分析" description="月次だけ確認。判定履歴、結果評価、重み診断、運用レポートをここへ集約。">
            <button type="button" onClick={saveDecisionSnapshot} className="rounded-2xl border border-emerald-300 bg-emerald-50 px-4 py-2 text-xs font-black text-emerald-700">現在判定を履歴保存</button>
            <button type="button" onClick={exportOperationalReportMarkdown} className="rounded-2xl border border-blue-300 bg-blue-50 px-4 py-2 text-xs font-black text-blue-700">運用レポートMD</button>
            <button type="button" onClick={exportRiskWeightDiagnosticsCsv} className="rounded-2xl border border-violet-300 bg-violet-50 px-4 py-2 text-xs font-black text-violet-700">重み診断CSV</button>
          </FocusHeader>
          <div className="grid gap-4 md:grid-cols-4"><MetricCard label="履歴" value={decisionHistory.length} subLabel="decisionHistory" tone="slate" /><MetricCard label="SUCCESS" value={decisionHistory.filter((item) => item.decisionAccuracy === 'SUCCESS').length} subLabel="成功評価" tone="emerald" /><MetricCard label="FAILURE" value={decisionHistory.filter((item) => item.decisionAccuracy === 'FAILURE').length} subLabel="失敗評価" tone="red" /><MetricCard label="NEED_DATA" value={riskWeightDiagnostics.filter((item) => item.recommendation === 'NEED_DATA').length} subLabel="重み診断不足" tone="amber" /></div>
        </section>
      )
    }
    if (activeDashboardTab === 'audit') {
      return (
        <section className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="監査ログ" description="異常時だけ確認。通常運用では常時見ない。">
            <button type="button" onClick={exportAuditLogCsv} className="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-xs font-black text-slate-700">監査CSV</button>
          </FocusHeader>
          <div className="space-y-2">{focusAuditItems.map((item, index) => <div key={`${item.changedAt}-${index}`} className="rounded-xl border border-slate-200 bg-white p-3 text-xs"><span className="font-black">{item.changedAt?.slice(0, 19).replace('T', ' ')}</span> / {item.code} / {item.fieldName} / {String(item.previousValue)} → {String(item.newValue)} / {item.changeSource}</div>)}</div>
        </section>
      )
    }
    if (activeDashboardTab === 'settings') {
      return (
        <section className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="設定・同期" description="ルール、リスク重み、Supabase同期、バックアップは必要時のみ開く。">
            <button type="button" onClick={exportBackendMigrationSpecJson} className="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-xs font-black text-slate-700">移行仕様JSON</button>
            <button type="button" onClick={exportExternalIntegrationPlanCsv} className="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-xs font-black text-slate-700">外部連携CSV</button>
          </FocusHeader>
          <div className="grid gap-4 md:grid-cols-3"><MetricCard label="安全モード" value={isReadOnlyMode ? 'READ' : 'EDIT'} subLabel="通常はREADに固定" tone={isReadOnlyMode ? 'emerald' : 'red'} /><MetricCard label="Supabase" value={isSupabaseConfigured ? '設定あり' : '未設定'} subLabel={cloudStatus} tone={isSupabaseConfigured ? 'sky' : 'amber'} /><MetricCard label="完全性" value={integritySummary.dataCompletenessScore || 0} subLabel="dataCompletenessScore" tone={(integritySummary.dataCompletenessScore || 0) >= 80 ? 'emerald' : 'amber'} /></div>
        </section>
      )
    }
    return (
      <section className="space-y-6">
        <div className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
          <FocusHeader title="今日見るべきこと" description="最初に見る画面。最大10件だけ表示し、通常はここから処理する。">
            <button type="button" onClick={exportDailyUpdatePackCsv} className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-2 text-xs font-black text-amber-700">日次Pack</button>
            <button type="button" onClick={exportBulkPasteTemplateTsv} className="rounded-2xl border border-cyan-300 bg-cyan-50 px-4 py-2 text-xs font-black text-cyan-700">貼付TSV</button>
          </FocusHeader>
          <div className="grid gap-4 md:grid-cols-4">
            <MetricCard label="要対応Top" value={focusNeedsAction.length} subLabel="最大10件表示" tone={focusNeedsAction.length ? 'red' : 'emerald'} />
            <MetricCard label="停止判定" value={focusBlockingDecisions.reduce((sum, key) => sum + (portfolioSummary.decisionCounts?.[key] || 0), 0)} subLabel="判定禁止" tone="amber" />
            <MetricCard label="入力候補" value={bulkInputSuggestions.length} subLabel="不足入力" tone={bulkInputSuggestions.length ? 'amber' : 'emerald'} />
            <MetricCard label="HIGH期限" value={checklistSummary.highOverdue} subLabel="運用タスク" tone={checklistSummary.highOverdue ? 'red' : 'emerald'} />
          </div>
          <div className="mt-5 space-y-3">{focusNeedsAction.map((stock, index) => <CompactStockRow key={stock.code} stock={stock} rank={index + 1} />)}{focusNeedsAction.length === 0 && <div className="rounded-2xl bg-emerald-50 p-6 text-sm font-bold text-emerald-700">今日の最優先銘柄はありません。チェックリストとデータ更新Packを確認。</div>}</div>
        </div>
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
            <h3 className="text-lg font-black text-slate-900">次にやる入力</h3>
            <div className="mt-3 space-y-2">{focusInputNeeds.slice(0, 5).map((item, index) => <div key={`${item.code}-${item.suggestedField}-${index}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"><span className="font-black">{item.code} {item.name}</span> / {item.suggestedField} / {item.requiredEvidence}</div>)}</div>
          </div>
          <div className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-sm">
            <h3 className="text-lg font-black text-slate-900">今日の作業順</h3>
            <div className="mt-3 space-y-2">{focusTaskRows.map((item) => <div key={item.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"><span className="font-black">{item.label}</span> / {item.doneCount}/{item.totalSteps} / {cadenceLabels[item.cadence]}</div>)}</div>
          </div>
        </div>
      </section>
    )
  ...
  if (activeDashboardTab !== 'legacy') {
    return (
      <div className={`min-h-screen bg-gradient-to-b from-sky-100 via-white to-slate-100 text-slate-900 ${isReadOnlyMode ? 'safe-readonly' : 'safe-editing'}`}>
        <div className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 px-6 py-3 shadow-sm backdrop-blur">
          <div className="mx-auto max-w-7xl">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="text-xl font-black text-slate-900">Portfolio Command Center</div>
                <div className="text-xs font-bold text-slate-500">通常は司令塔だけを見る。詳細・監査・設定はタブで隔離。</div>
              </div>
              <div className="flex flex-wrap gap-2">
                {isReadOnlyMode ? <button type="button" onClick={enableEditMode} className="rounded-2xl border border-red-300 bg-red-50 px-4 py-2 text-xs font-black text-red-700">EDITで編集解除</button> : <button type="button" onClick={disableEditMode} className="rounded-2xl border border-emerald-300 bg-emerald-50 px-4 py-2 text-xs font-black text-emerald-700">閲覧モードへ戻す</button>}
              </div>
            </div>
            <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
              {focusTabs.map((tab) => (
                <button key={tab.id} type="button" onClick={() => setActiveDashboardTab(tab.id)} className={`shrink-0 rounded-2xl border px-4 py-2 text-left text-xs font-black transition ${activeDashboardTab === tab.id ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-white'}`}>
                  <div>{tab.label}</div><div className={`text-[10px] ${activeDashboardTab === tab.id ? 'text-slate-300' : 'text-slate-400'}`}>{tab.hint}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <main className="mx-auto max-w-7xl space-y-6 px-6 py-6">
          {importMessage && <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-sm font-bold text-sky-700">{importMessage}</div>}
          {renderFocusedView()}
        </main>
      </div>
    )
